...
// @include        http://mikunopop.wiki.fc2.com/*
...
//		});
//	}
	if( location.href.match(/^https?\:\/\/jbbs\.livedoor\.jp\//) ){
		// bbs autolink
		$('dd').each( function () {
			var s = $(this).html();
	...
	else if( location.href.match(/^https?\:\/\/mikunopop\.wiki\.fc2\.com\//) ){
		// jump to direct links
		$('a').each( function () {
			var href = $(this).attr('href');
			if( href != null && href.match(/\/jump\//) ){
				href = href.replace(/^.*\/jump\//,"")
					.replace(/^http\//,"http://")
					.replace(/^https\//,"https://")
					.replace(/\?sid=.+$/,"");
